...
export class AsignaturaServicesService {
  ...
  asignaturasDetalleFiltradas: asignatura[] = [];
  asignaturasfiltradasDam: asignatura[] = [];
  asignaturasfiltradasDaw: asignatura[] = [];
  ciclosFiltrado: ciclo[] = []
  conocimientoFiltrado: conocimientos[] = [];
  imagenesFiltradas: conocimientos[] = [];
  ...
  getAsignaturasDam(): asignatura[] {
    this.asignaturasfiltradasDam.splice(0, this.asignaturasfiltradasDam.length)
    this.asignatura.forEach((tec) => {

      if (tec.ciclo.includes("DAM" || "DAM | DAW")) {
        this.asignaturasfiltradasDam.push(tec)

      }
    });
    return this.asignaturasfiltradasDam

  }


  getAsignaturasDaw(): asignatura[] {
    this.asignaturasfiltradasDaw.splice(0, this.asignaturasfiltradasDaw.length)
    this.asignatura.forEach((tec) => {

      if (tec.ciclo.includes("DAW" || "DAM | DAW")) {
        this.asignaturasfiltradasDaw.push(tec)

      }
    });
    return this.asignaturasfiltradasDaw

  }


  getAsignarurasDetalle(curso: number, nombre: string): asignatura[] {

    this.asignaturasDetalleFiltradas.splice(0, this.asignaturasDetalleFiltradas.length)
    this.asignatura.forEach((tec) => {
      if (tec.ciclo.includes(nombre) && tec.curso == curso) {
        this.asignaturasDetalleFiltradas.push(tec)
    ...
    return this.asignaturasDetalleFiltradas;
  }



  getConocimientosDetalle(curso: number, nombre: string): conocimientos[] {

    this.conocimientoFiltrado.splice(0, this.conocimientoFiltrado.length)
    this.asignatura.forEach((asg) => {
      if (asg.ciclo.includes(nombre) && asg.curso == curso) {
        asg.conocimientos.forEach(element => {
          this.conocimientoFiltrado.push(element)
        });
      }
    })
    return this.conocimientoFiltrado;
  }


  getAsignaturasFiltradas(nombre: string, ciclo: string): asignatura[] {
    this.asignaturasFiltradas.splice(0, this.asignaturasFiltradas.length)

    this.asignatura.forEach(element => {
      if (nombre == element.nombre && element.ciclo.includes(ciclo)) {
